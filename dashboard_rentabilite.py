...
def afficher_dashboard_rentabilite(situation: SituationActuelle, premier_bien: Optional[PremierBien], projet: Optional[NouveauProjet], resultats: dict):
    ...
    st.subheader("💰 Évolution des Cash-flows")
    
    fig_cashflow = make_subplots(
        rows=2, cols=1,
        subplot_titles=("Cash-flow annuel net", "Cash-flow cumulé"),
    ...
    fig_cashflow.add_trace(
        go.Scatter(
            ...
            y=df_projection['cash_flow_net'],
            mode='lines+markers',
            name='Cash-flow net annuel',
    ...
    # Cash-flow cumulé
    colors = ['red' if x < 0 else 'green' for x in df_projection['cash_flow_cumule']]
    fig_cashflow.add_trace(
        go.Scatter(
            x=df_projection['annee'],
            y=df_projection['cash_flow_cumule'],
            mode='lines+markers',
    ...
    fig_cashflow.add_hline(y=0, line_dash="dash", line_color="red", row=2, col=1)
    
    fig_cashflow.update_layout(
        height=600,
        title_text="Évolution des Cash-flows dans le temps",
        showlegend=True
    )
    
    fig_cashflow.update_xaxes(title_text="Année", row=2, col=1)
    fig_cashflow.update_yaxes(title_text="€", row=1, col=1)
    fig_cashflow.update_yaxes(title_text="€", row=2, col=1)
    
    st.plotly_chart(fig_cashflow, use_container_width=True)
    
    # 2. Évolution des rendements
    st.subheader("📊 Évolution des Rendements")
    
    fig_rendements = go.Figure()
    
    fig_rendements.add_trace(
        go.Scatter(
            x=df_projection['annee'],
            y=df_projection['rendement_brut'],
            mode='lines+markers',
            name='Rendement brut',
    ...
    fig_rendements.update_layout(
        title="Évolution des Rendements Locatifs",
        xaxis_title="Année",
        yaxis_title="Rendement (%)",
        height=400
    )
    
    st.plotly_chart(fig_rendements, use_container_width=True)
    
    # 3. Construction du patrimoine
